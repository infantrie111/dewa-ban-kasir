...
async function cetakStruk(transaksi) {
    // 1. Deteksi apakah berjalan di Android Native (APK)
    // Object 'Android' di-inject oleh MainActivity.java
    const isAndroidNative = window.Android && window.Android.cetakStruk;

    // 2. Jika BUKAN Android Native, wajib cek koneksi RawBT (Localhost)
    if (!isAndroidNative) {
        const printerReady = await cekStatusPrinter();
        if (!printerReady) return false;
    } else {
        console.log('📱 Mode: Android Hybrid App (Bluetooth Native)');
    }
    ...
    try {
        ...
        // === EKSEKUSI PENCETAKAN ===
        if (isAndroidNative) {
            // JALUR 1: APK HYBRID (Native Bluetooth)
            console.log('📱 Mengirim data ke Android Native...');

            // Panggil Interface Java (Fire and Forget)
            window.Android.cetakStruk(base64Data);

            // Beri feedback optimis (karena Java Toast handle error detail)
            showAlert('Mengirim ke Printer Bluetooth...', 'success');
            return true;
        } else {
            // JALUR 2: BROWSER BIASA (RawBT Localhost)
            console.log('🌍 Mengirim data ke RawBT Localhost...');
